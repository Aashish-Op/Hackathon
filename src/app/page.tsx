...
const WORKFLOW_STEPS = [
  {
    id: "01",
    title: "Vigilo Score",
    body: "11 signals - mock tests, activity gaps, applications, and skill mismatch - are collapsed into a 0-100 risk score every night.",
  },
  {
    id: "02",
    title: "Cluster Assignment",
    body: "KMeans groups each student into Placement Ready, At Risk, or Silent Dropout so the team knows where to focus first.",
  },
  {
    id: "03",
    title: "AI Nudge Delivery",
    body: "GPT-4o drafts personalized nudges for flagged students. TPCs send with one click, and every message is logged.",
  },
] as const;
...
function ClusterBadge({ cluster }: { cluster: Cluster }) {
  if (cluster === "Silent Dropout") {
    return <Badge variant="destructive">{cluster}</Badge>;
  }
  ...
  return <Badge variant="default">{cluster}</Badge>;
}
        ...
        <div className="mx-auto flex h-16 w-full max-w-[1100px] items-center justify-between px-6">
          ...
          <div className="flex items-center gap-4">
            {sessionLoading ? (
                ...
                {profileMenuOpen ? (
                  <div className="absolute right-0 top-11 z-50 w-56 border border-[rgba(26,26,26,0.18)] bg-[var(--paper)] p-3 shadow-[0_8px_24px_rgba(26,26,26,0.08)]">
                    ...
                    <div className="mt-3 border-t border-[rgba(26,26,26,0.12)] pt-2">
                      <Link href="/profile" className="block py-1 text-sm text-[var(--ink)]">
                        View Profile
                      </Link>
              ...
              <Link href="/auth" className="text-sm text-[var(--muted)]">
                Login
              </Link>
            )}
            <a
