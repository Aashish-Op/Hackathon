...
def _get_drive_or_404(drive_id: str) -> dict[str, Any]:
    client = get_supabase_client()
    rows = client.table("placement_drives").select("*").eq("id", drive_id).limit(1).execute().data or []
    if not rows:
...
def get_drive_details(
    ...
    client = get_supabase_client()
    drive = _get_drive_or_404(drive_id)
...
def update_drive(
    ...
    client = get_supabase_client()
    rows = client.table("placement_drives").update(update_payload).eq("id", drive_id).execute().data or []
